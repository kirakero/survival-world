...
func generate_lake():
	...
	var lake = generateLake(rng, simplex, extent, center, size)
...
func renderIsland(points: Array, extents: Rect2, simplex: OpenSimplexNoise):
	...
	mask = resize_and_fill(mask, (extents.size.x) * (extents.size.y), 0)
...
#	busy = true
	...
	var pipe = load("res://gen/TestPipe.gd").pipeline({}, self)
	...
	var im  = res['data']
	var img = Image.new()
	img.create_from_data( im.size.x, im.size.y, false, Image.FORMAT_L8, im.pa)
	var tex = ImageTexture.new()
	tex.create_from_image(img, 0)
	$Sprite.set_texture( tex )
#	busy = false
